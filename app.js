...
requestHandler
  ...
  .then(array => {
    ...
    htmlrenderHandler.renderFeaturePosts(array, deletePost);
    return lowestId;
  })
  .then(id => {
    requestHandler.getPosts(0, id).then(array => {
      ...
      htmlrenderHandler.renderOtherPost(array, deletePost);
    });
  });
...
document.getElementById("btn-filter-by-tag").addEventListener("click", e => {
  let filteredPost = postList.filterPostsByTags(
    htmlrenderHandler.getFiltersTags()
  );
...
document.getElementById("post-title-search-form").addEventListener("submit", e => {
    ...
    requestHandler.getFilteredPost(filter).then(arrayPost => {
      htmlrenderHandler.resetPostList();
      htmlrenderHandler.renderOtherPost(arrayPost);
    });
});
